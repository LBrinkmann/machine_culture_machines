...
def make_row(network, RT_tot, AT, NT, RT, RD, QD, n, n_steps, model_name, model_parameter):
    ...
    return {
        'Environment_ID': new_id, 'Solution_ID': solution_id,  'Network_ID': network['network_id'], 'Starting_Node': n, 'Depth': n_steps,
        'Total_Reward': RT_tot[n], 'Action_Trace': AT[:, n], 'Node_Trace': NT[:, n], 'Reward_Trace': RT[:, n], 'Reward_Diff': RD[:, n], 'Q_Diff': QD[:, n],
        'Model_Name': model_name, **model_parameter_capitalize}
